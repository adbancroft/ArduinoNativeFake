...
static inline void setupStreamFake(fakeit::Mock<TFake> &mock, std::ostream &oStream, std::istream &iStream)
{
    ...
    When(OverloadedMethod(mock, findUntil, bool(const char*, size_t, const char*, size_t))).AlwaysDo([&iStream](const char *target, size_t targetLen, const char *terminate, size_t termLen){
        ...
        if (!strTarget.empty() && !strTerminate.empty())
        {
            ...
            found = StreamDetail::search(searchTargets.begin(), searchTargets.end(), iStream);
        }
        else if (!strTarget.empty()) {
            found = StreamDetail::search(strTarget, iStream);
        }
        else if (!strTerminate.empty()) {
            found = StreamDetail::search(strTerminate, iStream);
        } else {
    ...
    When(OverloadedMethod(mock, parseInt, long(LookaheadMode, char))).AlwaysDo([&iStream](LookaheadMode mode, char ignore){
        // Create a locale with the custom ignore facet and
        // imbue the stringstream with the custom locale
        auto oldLocale = iStream.imbue(std::locale(std::locale::classic(), new StreamDetail::ignore_char_int_t(mode, ignore)));
        std::string i;
        iStream >> i;
        (void)iStream.imbue(oldLocale);
        try { 
    ...
    When(OverloadedMethod(mock, parseFloat, float(LookaheadMode, char))).AlwaysDo([&iStream](LookaheadMode mode, char ignore){
        // Create a locale with the custom ignore facet and
        // imbue the stringstream with the custom locale
        auto oldLocale = iStream.imbue(std::locale(std::locale::classic(), new StreamDetail::ignore_char_float_t(mode, ignore)));
        std::string i;
        iStream >> i;
        (void)iStream.imbue(oldLocale);
        try { 
    ...
    When(OverloadedMethod(mock, readBytes, size_t(char*, size_t))).AlwaysDo([&iStream](char *buffer, size_t length){
        (void)iStream.read(buffer, length);
        return iStream.gcount();
    });

    When(OverloadedMethod(mock, readBytesUntil, size_t(char, char*, size_t))).AlwaysDo([&iStream](char terminator, char *buffer, size_t length){
        char c = iStream.get();
        size_t index = 0;
        while ((index < length)
    ...
    When(Method(mock, readString)).AlwaysDo([&iStream](){
        std::string s;
        iStream >> s;
        return String(s.c_str());
    });

    When(Method(mock, readStringUntil)).AlwaysDo([&iStream](char terminator){
        std::string s;
        char c = iStream.get();
